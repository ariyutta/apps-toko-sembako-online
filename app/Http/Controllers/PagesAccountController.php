...
class PagesAccountController extends Controller
{
    public function accounts() {
        $title_user = 'My Account';
        $user = auth()->user()->id;
        $data = User::where('id', $user)->first();
        $notif_cart = DataKeranjang::where('status_notif','!=', 0)->count();

        return view('users.pages_auth.accounts', compact('title_user','data','notif_cart'));
    }

    public function edit_accounts() {
        $title_user = 'Edit Accounts';
        $user = auth()->user()->id;
        $data = User::where('id', $user)->first();
        $notif_cart = DataKeranjang::where('status_notif','!=', 0)->count();
        ...
        return view('users.pages_auth.edit_accounts', compact('title_user','data','agama','select_agama','pekerjaan','select_pekerjaan','notif_cart'));
    }
